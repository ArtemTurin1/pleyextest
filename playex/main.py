...
async def get_problems(subject: str = Query(None, description="Subject filter"), difficulty: str = Query(None, description="Difficulty filter")):
    problems = await services.get_problems(subject, difficulty)
    # Возвращаем задачи БЕЗ description, но С solution
    return [
        {
            'id': p.id,
            'title': p.title,
            'solution': p.solution,
            'subject': p.subject.value if hasattr(p.subject, 'value') else p.subject,
...
async def get_categories(subject: str = Query(None)):
    categories = await services.get_categories(subject)
    return [
        {
            'id': c.id,
            'name': c.name,
            'subject': c.subject.value if hasattr(c.subject, 'value') else c.subject
        }
        for c in categories
    ]

@app.get("/api/problems/by-category/")
async def get_problems_by_category(category_id: int = Query(None)):
    problems = await services.get_problems_by_category(category_id)
    return [
        {
            'id': p.id,
            'title': p.title,
